    ...
    for root, dirs, files in os.walk(file_path):
        ...
        for file in files:
            ...
            if ori_path and lab_path != None:
                ...
                ori_path= None
                lab_path= None
    output_path= ''.join(('training3\\result', str(go), '.csv'))
    # output_path= r'training3\result1.csv'
    df.to_csv(output_path, index= roku)
    print("Saved!")
    
if __name__ == '__main__':
    # main('training', 'shape', 'MeshVolume', 2, 1, 1)
    # main('training', 'shape', 'SurfaceVolumeRatio', 3, 2, 2)
    # main('training', 'shape', 'LeastAxisLength', 3, 2, 3)
    # main('training', 'shape', 'Maximum3DDiameter', 1, 2, 5)
    # main('training', 'glcm', 'Id', 1, 2, 6)
    # main('training', 'shape', 'Compactness2', 3, 2, 7)
    # main('training', 'shape', 'Maximum3DDiameter', 2, 2, 8)
    # main('training', 'shape', 'SurfaceArea', 1, 1, 9)
    # main('training', 'shape', 'MeshVolume', 1, 1, 10, False)
    # main('training', 'shape', 'MeshVolume', 1, 2, 11, False)
    # main('training', 'shape', 'MeshVolume', 3, 1, 12, False)
    # main('training', 'shape', 'MeshVolume', 3, 2, 13, False)
    main('training', 'shape', 'MeshVolume', 2, 2, 1.1, False)